<template>
  <div v-if="field.type === 'number'" class="py-1.5 px-1 relative" @click.stop>
    <select
  ...
  >
    ...
    >
      <option
        value="any"
        :selected="
          !field.preferences?.format || field.preferences.format === 'any'
        "
      >
        {{ t("any") }}
      </option>
      <option value="drawn" :selected="field.preferences?.format === 'drawn'">
        {{ t("drawn") }}
      </option>
      <option value="typed" :selected="field.preferences?.format === 'typed'">
        {{ t("typed") }}
      </option>
    </select>
    <label
  ...
  <li v-if="withRequired && field.type != 'stamp'" @click.stop>
  ...
  <li v-if="field.type == 'checkbox'" @click.stop>
    <label class="cursor-pointer py-1.5">
      <input
        ...
        class="toggle toggle-xs"
        @update:model-value="
          [(field.default_value = $event), (field.readonly = $event), save()]
        "
      />
        ...
        <template v-if="template.schema.length > 1"
          >{{
            template.schema.findIndex(
              (item) => item.attachment_uuid === area.attachment_uuid
            ) + 1
          }}-</template
        >{{ area.page + 1 }}
      </a>
  ...
  <li
    v-if="
      ...
      ['date', 'signature', 'initials', 'text', 'cells'].includes(field.type)
    "
  >
    <a
      href="#"
...
export default {
  ...
  computed: {
    ...
    dateFormats() {
      ...
      if (
        Intl.DateTimeFormat().resolvedOptions().timeZone?.includes("Seoul") ||
      ...
      this.template.documents.forEach((attachment) => {
        ...
        for (let page = 0; page <= numberOfPages - 1; page++) {
          if (
            !field.areas.find(
              (area) =>
                area.attachment_uuid === attachment.uuid && area.page === page
            )
          ) {
            field.areas.push({
