  ...
  <div>
    <div v-if="selectedSheetIndex === null && spreadsheet">
      <form
        @submit.prevent="
          [
            (selectedSheetIndex = $refs.selectWorksheet.value),
            buildDefaultMappings(),
          ]
        "
      >
        <label class="label"> Select Worksheet </label>
        <select ref="selectWorksheet" class="base-select">
          <option
        ...
        <button class="base-button mt-4 w-full">Open</button>
      </form>
    </div>
    <div v-else-if="selectedSheetIndex !== null">
      <div v-for="submitter in submitters" :key="submitter.uuid" class="mb-4">
        <div
        ...
        <div class="flex">
          <div class="relative w-full py-2 px-2 text-sm">Recipient field</div>
        ...
        <div
          v-for="mapping in mappings.filter(
            (m) => m.submitter_uuid === submitter.uuid
          )"
          :key="mapping.uuid"
          class="mb-2"
        >
          <div class="flex">
            ...
            >
              <option disabled value="" :selected="!mapping.field_name">
                Select Field
              </option>
              <option
            ...
            <div class="w-full relative">
              ...
              >
                <span
                  ...
                  :data-tip="
                    [0, 1, 2]
                      .map((i) => rows[i]?.[mapping.column_index] ?? '---')
                      .join('\n')
                  "
                >
                  <button
            ...
            <div class="flex items-center pl-1">
              <span class="tooltip tooltip-top" data-tip="Remove">
                <button
                  :disabled="
                    mappings.filter((m) => m.submitter_uuid === submitter.uuid)
                      .length < 2
                  "
      ...
      >
        ...
        <template v-if="submissionsData.length >= 1000"> / 1000 </template>
      </div>
    </div>
    <div v-else>
      <div
      ...
      <div class="text-center mt-2">
        ...
        <a
          ...
          >download</a
        >
        a spreadsheet to fill and import
      </div>
    </div>
  </div>
</template>
      ...
      const templateFields = this.template.fields.filter((field) => {
        return (
          ...
          this.defaultFields.every(
            (f) => field.name?.toLowerCase() !== f.name?.toLowerCase()
          )
        );
      });
    ...
    buildDefaultMappings() {
      this.submitters.forEach((submitter) => {
        ...
        if (
          !this.mappings.some(
            (m) =>
              m.field_name.toLowerCase() === "name" &&
              m.submitter_uuid === submitter.uuid
          )
        ) {
        ...
        if (
          !this.mappings.some(
            (m) =>
              m.field_name.toLowerCase() === "email" &&
              m.submitter_uuid === submitter.uuid
          )
        ) {
          this.mappings.unshift({
